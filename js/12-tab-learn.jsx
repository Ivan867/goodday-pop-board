  ...
  if (pageMode === "idea") {
    return (
      <div>
        ...
        {window.LazyTab ? React.createElement(window.LazyTab, { tabKey:"idea", compProps:{ embedded:true } }) : null}
      </div>
    );
  }
...
// 番号 → その番号で開く店舗
const ORDER_STORES = [
  { pin:"5", code:"kisuki", name:"木次店" },
  { pin:"7", code:"hokubu", name:"北部店" },
];
...
function OrderTab() {
  ...
  const tryUnlock = () => {
    ...
    if (hit) {
      setStore(hit); setUnlocked(true); setPwErr("");
      try { sessionStorage.setItem("orderStore", hit.code); } catch(e) {}
  ...
  const ensureSheet = async () => {
    ...
    const created = await api.createSheet({ week_start: wkKey, store_code: storeCode, author: localStorage.getItem("lastAuthor") || null });
  ...
  const [savedDays, setSavedDays] = useState(() => {
    try { return JSON.parse(localStorage.getItem("orderSavedDays") || "{}"); } catch(e) { return {}; }
  });
  ...
  const pastDaysOfWeek = () => {
    ...
    SHEET_DAYS.forEach(([k], i) => {
      const d = new Date(wkStart); d.setDate(d.getDate() + i); d.setHours(0,0,0,0);
      if (d < today) out.push({ key:k, date:d, ymd:oiYmd(d) });
    });
    return out;
  };
  ...
  const recCount = () => {
    let n = 0;
    pastDaysOfWeek().forEach(({ key, ymd }) => {
  ...
  const saveWeekToLogs = async () => {
    ...
    if (!days.length) { setRecMsg("記録できる日がありません"); return; }
    setRecBusy(true); setRecMsg("");
    try {
      const logs = [];
      days.forEach(({ key, ymd }) => {
        rows.filter(r => r[key] != null && r[key] !== "").forEach(r => {
          logs.push({ item_id: r.item_id, ordered_on: ymd, qty: Number(r[key]), store_code: storeCode,
            item_name: r.item_name, unit: r.unit || null, maker: r.maker || null, price: r.price ?? null,
            note: r.memo || null, author: localStorage.getItem("lastAuthor") || null });
        });
      });
      if (!logs.length) { setRecMsg("記録するものがありません"); setRecBusy(false); return; }
      await api.saveWeekLogs(logs);
      const mark = { ...savedDays };
      days.forEach(d => { mark[d.ymd] = true; });
      setSavedDays(mark);
      try { localStorage.setItem("orderSavedDays", JSON.stringify(mark)); } catch(e) {}
                  ...
                  {recCount() > 0 && (
                    <div style={{ background:"#fff", border:"1.5px solid #cfe0d8", borderRadius:11, padding:"12px 13px", marginBottom:12 }}>
                      ...
                      <button onClick={saveWeekToLogs} disabled={recBusy}
                        style={{ width:"100%", border:"none", background: recBusy ? "#ccc" : "#3f9e63", color:"#fff", borderRadius:9, padding:"11px", fontSize:13, fontWeight:900, cursor:"pointer" }}>
                        {recBusy ? "記録中…" : `${recCount()}件を記録する`}
                      </button>
                    </div>
                  )}
                  {recMsg && (
                    <div style={{ fontSize:12, fontWeight:800, color: recMsg.includes("できません") ? "#b3261e" : "#2c6b45",
                      background: recMsg.includes("できません") ? "#fdeceb" : "#eaf6ee", borderRadius:8, padding:"9px 11px", marginBottom:12 }}>{recMsg}</div>
                  )}
