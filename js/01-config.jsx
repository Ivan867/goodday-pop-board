...
async function sbFetch(path, { method = "GET", body, prefer, headers = {} } = {}) {
  ...
  const r = await fetch(`${SB_URL}${path}`, { method, headers: h(extra), body: body !== undefined ? JSON.stringify(body) : undefined });
...
const api = {
  ...
  async upload(file) {
    const blob = await new Promise(res => {
      ...
      img.onload = () => {
        ...
        const max = 1200;
        let w = img.width, wh = img.height;
        if (w > max) { wh = Math.round(wh*max/w); w = max; }
        c.width = w; c.height = wh;
  ...
  // ── blog_posts ──
  async listPosts() { return sbJson(`/rest/v1/blog_posts?select=*&order=created_at.desc`); },
  async insertPost(data) { return sbOne(`/rest/v1/blog_posts`, { method:"POST", body:data, prefer:"return=representation" }); },
  async incrementViews(id, currentViews) { return sbOne(`/rest/v1/blog_posts?id=eq.${id}`, { method:"PATCH", body:{ views:(currentViews||0)+1 }, prefer:"return=representation" }); },
  async delPost(id) { await sbFetch(`/rest/v1/blog_posts?id=eq.${id}`, { method:"DELETE" }); },

  // ── pop_comments ──
  async listComments(popId) { return sbJson(`/rest/v1/pop_comments?pop_id=eq.${popId}&order=created_at.asc&select=*`); },
  async addComment(popId, storeName, comment) { return sbOne(`/rest/v1/pop_comments`, { method:"POST", body:{ pop_id: popId, store_name: storeName, comment }, prefer:"return=representation" }); },
  ...
  async verifyPassword(purpose, password) {
    const v = await sbJson(`/rest/v1/rpc/verify_password`, { method:"POST", body:{ p_purpose: purpose, p_password: password } });
    if (v === true) PW_CACHE[purpose] = password;
    return v === true;
  },

  // ── tray_presets：包材・トレーのカテゴリ（プリセット） ──
  async listTrayPresets() { return sbJson(`/rest/v1/tray_presets?select=*&order=sort_order.asc`); },
  async createTrayPreset(name, sort_order) { return sbOne(`/rest/v1/tray_presets`, { method:"POST", body:{ name, sort_order, items: [] }, prefer:"return=representation" }); },
  async updateTrayPreset(id, patch) { await sbFetch(`/rest/v1/tray_presets?id=eq.${id}`, { method:"PATCH", body:patch, prefer:"return=minimal" }); },
  async deleteTrayPreset(id) { await sbFetch(`/rest/v1/tray_presets?id=eq.${id}`, { method:"DELETE" }); },
  ...
  async getMemo() {
    try {
      ...
      return d.length > 0 ? d[0] : { id: 1, text: "" };
    } catch(e) { return { id: 1, text: "" }; }   // 取得失敗時は空メモ（従来挙動を踏襲）
  },
  async saveMemo(text) {
    const result = await sbJson(`/rest/v1/production_notes?id=eq.1`, { method:"PATCH", body:{ text, updated_at: new Date().toISOString() }, prefer:"return=representation" });
    return result.length > 0 ? result[0] : { id: 1, text };
  }
};



;Object.assign(window, { POP_COLS, POP_LIMIT, PW_CACHE, SB_KEY, SB_URL, api, h, sbFetch, sbJson, sbOne });
...
function seasonalEventsFor(year) {
  const mk = (y, m, d, name, food) => ({ date: new Date(y, m-1, d), name, food });
  const nthSunday = (y, m, n) => { const first = new Date(y, m-1, 1); const off = (7 - first.getDay()) % 7; return new Date(y, m-1, 1 + off + (n-1)*7); };
  const USHI = { 2026:[[7,26,"土用の丑"]], 2027:[[7,21,"土用の丑"],[8,2,"土用の二の丑"]], 2028:[[7,27,"土用の丑"]] };
  const HIGAN = { 2026:[[3,17],[9,20]], 2027:[[3,18],[9,20]], 2028:[[3,17],[9,19]] };
  const list = [
    mk(year,1,1,"お正月","刺身・おせち"), mk(year,2,3,"節分","恵方巻"), mk(year,3,3,"ひな祭り","ちらし寿司・はまぐり"),
    mk(year,5,5,"こどもの日","お祝い・ちらし"), mk(year,7,2,"半夏生","タコ"), mk(year,7,7,"七夕","ちらし・そうめん"),
    mk(year,8,13,"お盆入り","刺身・お供え"), mk(year,10,31,"ハロウィン",null), mk(year,12,22,"冬至",null),
    mk(year,12,24,"クリスマス","オードブル・寿司"), mk(year,12,31,"大晦日","刺身・寿司"),
  ];
  list.push({ date:nthSunday(year,5,2), name:"母の日", food:"ごちそう・お祝い" });
  list.push({ date:nthSunday(year,6,3), name:"父の日", food:"ごちそう・晩酌のあて" });
...
const scrollerY = () => { const el = scroller(); return el ? el.scrollTop : 0; };
const scrollerTop = (smooth) => { const el = scroller(); if (el) el.scrollTo({ top:0, behavior: smooth ? "smooth" : "auto" }); };
...
function arcPositions(n, radius, fromDeg, toDeg) {
  ...
  for (let i=0;i<n;i++){
    ...
    arr.push({ tx: Math.cos(rad)*radius, ty: -Math.sin(rad)*radius });
  }
  return arr;
}
...
;Object.assign(window, { arcPositions, formatDate, scroller, scrollerTop, scrollerY, seasonalEventsFor, timeAgo });
    ...
    <div style={{ maxWidth:1080, margin:"0 auto", padding:"10px 16px 84px" }}>
      ...
      {loading ? (
        ...
        allPops.length === 0 ? (
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:800, color:"var(--sub)", marginBottom:12, paddingLeft:2 }}>みんなのポップ（{allPops.length}）</div>
...
function FloorPhotoTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const data = await api.listFloorPhotos("", ""); setPhotos(data); }
    catch(e) { console.error(e); }
    finally { setLoading(false); }
  }, []);
  ...
  const handleDelete = async () => {
    if (fDeleting) return;
    setFDeleting(true); setPwError("");
    try {
      const ok = await api.verifyPassword("delete", pwInput);
      if (!ok) { setPwError("パスワードが違います"); setPwInput(""); setFDeleting(false); return; }
      await api.delFloorPhoto(delTarget.id);
      setPhotos(p => p.filter(x => x.id !== delTarget.id));
      setDelTarget(null); setSel(null); setPwInput(""); setPwError("");
    } catch(e) { alert("削除に失敗しました"); }
    finally { setFDeleting(false); }
  };
    ...
    <div className="min-vh" style={{ background:"var(--bg)" }}>
        ...
        <div style={{ maxWidth:1080, margin:"0 auto", padding:"16px 16px 40px" }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:"center", padding:80, color:"var(--faint)" }}>
              ...
              <div style={{ fontWeight:800, fontSize:16, color:"var(--sub)" }}>写真がまだありません</div>
              <div style={{ fontSize:13, marginTop:6 }}>「＋ 投稿」ボタンから売場写真を共有しましょう</div>
            </div>
          ) : (
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(158px, 1fr))", gap:12, alignItems:"start" }}>
              {filtered.map((photo,i)=>(
                <div key={photo.id}
                  ...
                  <div style={{ padding:"10px 12px" }}>
                    <div style={{ fontWeight:800, fontSize:13, marginBottom:3 }}>{photo.store_name}</div>
                    {photo.comment && <div style={{ fontSize:12, color:"var(--text)", marginBottom:3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{photo.comment}</div>}
                    <div style={{ fontSize:11, color:"var(--faint)" }}>{timeAgo(photo.created_at)}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {mode === "compare" && (
        <div style={{ maxWidth:1400, margin:"0 auto", padding:"16px 16px 40px" }}>
          ...
          {loading ? (
            <div style={{ textAlign:"center", padding:60, color:"var(--faint)" }}>読み込み中...</div>
          ) : (
              ...
              {compareData.map(({store, photos:storePhotos})=>(
                <div key={store} style={{ background:"white", borderRadius:14, overflow:"hidden", boxShadow:"0 2px 10px rgba(0,0,0,0.07)" }}>
                  ...
                  <div style={{ background:"linear-gradient(180deg,#e7f1fa,#d3e5f4)", padding:"10px 14px" }}>
                    ...
                    <div style={{ fontSize:11, color:"rgba(255,255,255,0.7)", marginTop:2 }}>{storePhotos.length}枚</div>
                  </div>

                  {storePhotos.length === 0 ? (
                    <div style={{ padding:"30px 14px", textAlign:"center", color:"var(--faint)" }}>
                    ...
                    <div>
                      ...
                      {storePhotos[0].comment && (
                        <div style={{ padding:"8px 12px", fontSize:12, color:"var(--text)", borderBottom:"1px solid var(--line)", lineHeight:1.5 }}>
                          {storePhotos[0].comment}
                        </div>
                      )}
                      {/* 過去の写真サムネイル */}
      ...
      {/* ── アップロードモーダル ── */}
      {showUp && <FloorUploadModal onClose={()=>setShowUp(false)} onSuccess={p=>{setPhotos(prev=>[p,...prev]);setShowUp(false);}} />}
          ...
          <div style={{ background:"white", borderRadius:20, width:"100%", maxWidth:500, maxHeight:"90vh", overflowY:"auto", animation:"fadeUp 0.2s ease" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:"16px 20px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                <div>
                  <div style={{ fontSize:17, fontWeight:900 }}>{sel.store_name}</div>
                  {sel.author && <div style={{ fontSize:13, color:"var(--sub)", marginTop:2 }}>投稿者：{sel.author}</div>}
                </div>
                <div style={{ fontSize:12, color:"var(--faint)" }}>{formatDate(sel.created_at)}</div>
              </div>
              {sel.comment && (
                <div style={{ fontSize:14, color:"var(--text)", background:"var(--bg)", borderRadius:10, padding:"10px 12px", lineHeight:1.7, whiteSpace:"pre-wrap", marginBottom:14 }}>
                  {sel.comment}
                </div>
              )}
              ...
              {/* 削除エリア */}
              {delTarget?.id === sel.id ? (
                <div style={{ background:"#fff5f5", border:"2px solid #ffd0d0", borderRadius:12, padding:"14px", marginBottom:12 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:"#d05050", marginBottom:8 }}>本当に削除しますか？</div>
                  <div style={{ fontSize:12, color:"var(--sub)", marginBottom:8 }}>ヒント：本社の郵便番号</div>
                  <div style={{ display:"flex", gap:8 }}>
                    <input type="password" value={pwInput} onChange={e=>{ setPwInput(e.target.value); setPwError(""); }}
                      onKeyDown={e=>e.key==="Enter"&&handleDelete()} placeholder="パスワード" autoFocus
                      style={{ flex:1, padding:"9px 12px", border:`2px solid ${pwError?"var(--primary)":"#ffd0d0"}`, borderRadius:8, fontSize:14, outline:"none" }} />
                    <button onClick={handleDelete} disabled={fDeleting} style={{ padding:"9px 14px", background: fDeleting ? "#dba0a0" : "#d05050", color:"white", border:"none", borderRadius:8, fontSize:13, fontWeight:700, cursor: fDeleting ? "default" : "pointer" }}>{fDeleting ? "確認中…" : "削除"}</button>
                    <button onClick={()=>{ setDelTarget(null); setPwInput(""); setPwError(""); }}
                      style={{ padding:"9px 12px", background:"var(--chip)", color:"var(--text)", border:"none", borderRadius:8, fontSize:13, fontWeight:700, cursor:"pointer" }}>戻る</button>
                  </div>
                  {pwError && <div style={{ fontSize:12, color:"var(--primary)", marginTop:6 }}>{pwError}</div>}
                </div>
              ) : (
                <button onClick={()=>setDelTarget(sel)}
                  style={{ width:"100%", background:"#fff5f5", border:"2px solid #ffd0d0", borderRadius:12, padding:"10px", fontSize:14, fontWeight:700, cursor:"pointer", color:"#d05050" }}>
                  削除
                </button>
              )}
            </div>
...
function FloorUploadModal({ onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      const image_url = await api.upload(file);
      const photo = await api.insertFloorPhoto({ store_name: store, category, image_url, author: author.trim(), comment: comment.trim() });
      onSuccess(photo);
    } catch(e) { setError("エラー: " + e.message); }
    finally { setLoading(false); }
  };
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000 }}>
      <div style={{ background:"white", borderRadius:"22px 22px 0 0", padding:"8px 24px calc(22px + env(safe-area-inset-bottom))", width:"100%", maxWidth:560, maxHeight:"92vh", overflowY:"auto", animation:"sheetUp .32s cubic-bezier(.16,1,.3,1)" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
          ...
          <div>
            <div style={{ fontSize:12, fontWeight:700, color:"var(--text)", marginBottom:5 }}>お名前 <span style={{ color:"var(--primary)" }}>*</span></div>
            <input value={author} onChange={e=>setAuthor(e.target.value)} placeholder="例：山田 太郎"
              style={{ width:"100%", padding:"9px 12px", border:"2px solid var(--line)", borderRadius:8, fontSize:14, outline:"none" }} />
          </div>
          <div>
            <div style={{ fontSize:12, fontWeight:700, color:"var(--text)", marginBottom:5 }}>コメント <span style={{ fontWeight:400, color:"var(--faint)" }}>（任意）</span></div>
            <textarea value={comment} onChange={e=>setComment(e.target.value)} placeholder="売り場の状況や工夫など..." rows={2}
              style={{ width:"100%", padding:"9px 12px", border:"2px solid var(--line)", borderRadius:8, fontSize:13, resize:"vertical", fontFamily:"inherit", outline:"none" }} />
          </div>
          <div>
          ...
          <button onClick={submit} disabled={loading}
            style={{ background:"#2f6fb0", color:"white", border:"none", borderRadius:12, padding:"13px", fontSize:15, fontWeight:900, cursor:"pointer", opacity:loading?0.6:1 }}>
            {loading ? "投稿中..." : "投稿する"}
          </button>
        </div>
      </div>
...
function useDragList(list, setLocal, persist) {
  ...
  const st = React.useRef({ list, idx:-1 });
  st.current.list = list;
  const [dragIdx, setDragIdx] = useState(-1);

  const down = (i) => (e) => {
    e.preventDefault();
    st.current.idx = i; setDragIdx(i);
  ...
  const move = (e) => {
    ...
    if (target !== di && target >= 0) {
      ...
      st.current.list = arr; st.current.idx = target;
      setDragIdx(target); setLocal(arr);
    }
  };
  const up = (e) => {
    const di = st.current.idx;
    st.current.idx = -1; setDragIdx(-1);
    if (di >= 0) persist(st.current.list);
  };
  return { containerRef, dragIdx, down, move, up };
}
...
const TRAY_CATS = [
  ...
  { key:"tray",   label:"包材・トレー",  codes:["876101","874603"] },
  { key:"frozen", label:"冷食・たれ",    codes:["220001","300701","200401","180101","402203","420201","680701","301403","420401","101"] },
  { key:"shizai", label:"資材",         codes:["880401"] },
  { key:"fresh",  label:"生鮮",         codes:["990001","999901","999902"] },
];



;Object.assign(window, { FloorPhotoTab, FloorUploadModal, TRAY_CATS, useDragList });
      ...
      {open && (
        <>
          ...
          <div style={{ position:"absolute", top:"calc(100% + 8px)", right:0, zIndex:211, background:"#fff", borderRadius:14, boxShadow:"0 8px 30px rgba(0,0,0,0.18)", padding:"13px 15px", width:230, animation:"fadeUp .2s ease" }}>
            ...
            <div style={{ fontSize:10, color:"var(--faint)", marginTop:6 }}>データ：Open-Meteo</div>
          </div>
        </>
      )}
    </div>
...
function CalendarTab() {
  ...
  const monthEvents = allEv.filter(e => e.date.getFullYear()===ym.y && e.date.getMonth()===ym.m).sort((a,b)=>a.date-b.date);
        ...
        <div style={{ background:"#fff", border:"1px solid var(--line)", borderRadius:14, padding:"10px 8px 8px", marginBottom:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:2 }}>
            {cells.map((d,i) => {
              ...
              return (
                <div key={i} style={{ minHeight:52, borderRadius:8, padding:"3px 2px", background: isToday ? "var(--soft)" : "transparent", border: isToday ? "1.5px solid var(--primary)" : "1px solid transparent", display:"flex", flexDirection:"column", alignItems:"center" }}>
                  <span style={{ fontSize:12.5, fontWeight: isToday?900:700, color: ev ? "var(--primary)" : wd===0?"#c0392b":wd===6?"#2f6fb0":"var(--text)" }}>{d}</span>
        ...
        {monthEvents.length === 0 ? (
          ...
          <div key={i} style={{ display:"flex", alignItems:"center", gap:10, background:"#fff", border:"1px solid var(--line)", borderRadius:12, padding:"11px 13px", marginBottom:8 }}>
            <div style={{ flexShrink:0, width:44, textAlign:"center" }}>
              <div style={{ fontSize:16, fontWeight:900, color:"var(--primary)", lineHeight:1 }}>{e.date.getDate()}</div>
              <div style={{ fontSize:10, color:"var(--sub)", fontWeight:700 }}>{jp[e.date.getDay()]}</div>
            </div>
            <div style={{ minWidth:0, flex:1 }}>
              <div style={{ fontSize:14, fontWeight:900, color:"var(--ink)" }}>{e.name}</div>
...
function AdminTab({ onNoticeChange }) {
...
;Object.assign(window, { AdminTab, ArchiveTab, NoticeAdmin, RequestTab });
...
function App() {
  ...
  return (
    <div id="app-scroll" className="min-vh" style={{ background:"var(--bg)", paddingBottom:"calc(62px + env(safe-area-inset-bottom))" }}>
      ...
      {tab==="admin"  && <AdminTab onNoticeChange={setNotice} />}
