...
function BarcodeTab() {
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([loadScriptOnce(XLSX_SRC), loadScriptOnce(JSBARCODE_SRC)]).then(() => {
      if (alive) setLibsReady(true);
  ...
  const findCol = (header, cands) => {
    for (const c of cands) {
      const i = header.findIndex(x => String(x).replace(/\s/g, "") === c);
      if (i >= 0) return i;
    }
    for (const c of cands) {
      const key = c.slice(0, 6);
      const i = header.findIndex(x => String(x).includes(key));
      if (i >= 0) return i;
    }
    return -1;
  };
  ...
  const addItem = it => {
    if (inList(it.bcode)) return;
    setList(prev => [...prev, it]);
  };
  ...
  const importCfg = file => {
    ...
    reader.onload = e => {
      try {
        ...
        const clean = items.filter(x => x && x.bcode).map(x => ({
          name: x.name || "",
          pcode: x.pcode || "",
          bcode: String(x.bcode),
          kubun: x.kubun || "",
          haccyu: x.haccyu || ""
        }));
        setList(clean);
        if (parsed && parsed.name) setListName(parsed.name);
        setErr("");
      } catch (ex) {
        setErr("プリセットを読み込めませんでした");
      }
    };
  ...
  useEffect(() => {
    api.listSharedPresets().then(d => {
      setShared(d);
      setSharedLoad(false);
    }).catch(() => setSharedLoad(false));
  }, []);
  const cleanItems = items => (items || []).filter(x => x && x.bcode).map(x => ({
    name: x.name || "",
    pcode: x.pcode || "",
    bcode: String(x.bcode),
    kubun: x.kubun || "",
  ...
  const loadShared = p => {
    ...
    setListName(p.name || p.data && p.data.name || "");
    setErr("");
    scrollerTop(true);
  };
  const downloadShared = p => {
    ...
    const data = {
      ...
      v: 2,
      name: p.name || "preset",
      items: cleanItems(p.data && p.data.items)
    };
    const blob = new Blob([JSON.stringify(data, null, 2)], {
      type: "text/plain;charset=utf-8"
    });
  ...
  const uploadShared = async files => {
    ...
    for (const f of arr) {
      try {
        ...
        const row = await api.insertSharedPreset({
          name: nm,
          data: {
            type: "tray-list",
            v: 2,
  ...
  const unlockShAdmin = async () => {
    try {
      ...
      setShPwErr("確認に失敗しました");
    }
  };
  const toggleShSel = id => setShSel(s => {
    const n = new Set(s);
  ...
  const bulkDelShared = async () => {
    ...
    setShBusy(true);
    try {
      for (const id of Array.from(shSel)) {
        await api.delSharedPreset(id);
      }
  ...
  const [useTags, setUseTags] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("bcUseTags") || "{}");
    } catch (e) {
      return {};
    }
  });
  ...
  const saveUseTags = next => {
    setUseTags(next);
    try {
      localStorage.setItem("bcUseTags", JSON.stringify(next));
    } catch (e) {}
  };
  ...
  const _emUnits = name => {
    let u = 0;
    for (const c of String(name || "")) u += /[\u3040-\u30ff\u3400-\u9fff\uff00-\uffef\u2e80-\u303f]/.test(c) ? 1.0 : 0.55;
    return u || 1;
  };
  ...
  const renderPages = forPrint => pages.map((pg, pi) => /*#__PURE__*/React.createElement("div", {
    key: pi,
    className: "bc-page",
    style: forPrint ? {
      breakAfter: "page",
  ...
  }, !forPrint && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      top: -9,
      left: 14,
    ...
    style: {
      ...
      gridTemplateRows: forPrint ? `repeat(${ROWS}, 40mm)` : `repeat(${ROWS}, minmax(0,1fr))`,
      gridAutoFlow: "column",
      gap: forPrint ? "3mm" : "2mm",
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        paddingTop: comp ? forPrint ? "7mm" : "17px" : undefined,
        breakInside: "avoid",
        display: "grid",
        minWidth: 0,
        minHeight: 0,
        gridTemplateRows: showName ? "auto auto auto" : "auto auto",
        alignContent: "center",
        justifyItems: "center",
      ...
      style: {
        ...
        color: comp.color,
        fontWeight: 900,
        fontSize: forPrint ? "8pt" : "8px",
        borderRadius: 3,
        padding: forPrint ? "0.3mm 1.5mm" : "1px 4px",
        lineHeight: 1.4,
        whiteSpace: "nowrap"
      }
      ...
      style: {
        position: "absolute",
        top: comp ? forPrint ? "1.2mm" : "3px" : forPrint ? "1mm" : "2px",
      ...
      style: {
        fontSize: forPrint ? nameFitPt(it.name) + "pt" : "9.5px",
        fontWeight: 800,
        lineHeight: 1.2,
        color: "#000",
        width: "100%",
      ...
      style: {
        fontSize: forPrint ? "15pt" : "9px",
        fontWeight: 700,
        letterSpacing: forPrint ? "0.4mm" : "0.2px",
        color: "var(--ink)",
        lineHeight: 1
      }
    }, it.bcode));
  })), listName && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      right: forPrint ? "4mm" : "8px",
      bottom: forPrint ? "2mm" : "7px",
      fontSize: forPrint ? "20pt" : "18px",
      fontWeight: 800,
      color: "var(--text)",
      letterSpacing: ".02em"
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      border: "none",
      background: list.length ? ACCENT : "#ccc",
      color: "#fff",
      borderRadius: 9,
      padding: "9px 14px",
      fontSize: 13,
    ...
    onChange: e => importCfg(e.target.files[0]),
    style: {
      display: "none"
    }
    ...
    return rows.length === 0 ? /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        key: p.id,
        onClick: shAdmin ? () => toggleShSel(p.id) : undefined,
        style: {
          display: "flex",
          alignItems: "center",
          gap: 8,
          background: shAdmin && checked ? "#eaf2ff" : isPdf ? "#fef6ef" : "#fafbfc",
          border: `1px solid ${shAdmin && checked ? "#8ab4ff" : isPdf ? "#f4d9c4" : "#eef0f2"}`,
          borderRadius: 10,
          padding: "8px 11px",
          cursor: shAdmin ? "pointer" : "default"
        }
      }, shAdmin && /*#__PURE__*/React.createElement("input", {
        type: "checkbox",
        checked: checked,
        readOnly: true,
        style: {
      ...
      }, "DL")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
        onClick: () => loadShared(p),
        style: {
          border: "none",
          background: ACCENT,
          color: "#fff",
  ...
  }, shTab === "pdf" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    ...
    multiple: true,
    onChange: e => uploadSharedPdf(e.target.files),
    style: {
      display: "none"
    }
  }), /*#__PURE__*/React.createElement("div", {
    ...
    accept: ".txt,.json,text/plain",
    multiple: true,
    onChange: e => uploadShared(e.target.files),
    style: {
      display: "none"
    }
  }), /*#__PURE__*/React.createElement("div", {
  ...
  }, !shAdmin && !shPwOpen && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setShPwOpen(true);
      setShPwErr("");
    },
  ...
  }, shPwErr)), shAdmin && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 8,
  ...
  }), !shConfirm ? /*#__PURE__*/React.createElement("button", {
    onClick: () => shSel.size > 0 && setShConfirm(true),
    disabled: shSel.size === 0,
    style: {
      border: "none",
      background: shSel.size ? "#c62828" : "#e6bcbc",
      color: "#fff",
      borderRadius: 8,
      padding: "8px 13px",
      fontSize: 13,
  ...
  }, shBusy ? "削除中…" : "実行"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setShConfirm(false),
    style: {
      border: "none",
      background: "none",
      color: "var(--sub)",
  ...
  }, list.length)), list.length > 0 && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      if (confirm("リストを空にしますか？")) setList([]);
    },
    style: {
      border: "none",
      background: "none",
    ...
    style: {
      ...
      alignItems: "center",
      gap: 10,
      background: drag.dragIdx === i ? "#eef4ff" : "#fafbfc",
      border: "1px solid #eef0f2",
      borderRadius: 10,
      padding: "9px 11px",
      boxShadow: drag.dragIdx === i ? "0 4px 14px rgba(0,0,0,0.12)" : "none"
    }
  }, /*#__PURE__*/React.createElement("button", {
    onPointerDown: drag.down(i),
  ...
  }, it.name || "（名称なし）"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 6,
  ...
  }, it.bcode), (useTags[it.bcode] || []).map(t => /*#__PURE__*/React.createElement("span", {
    key: t,
    style: {
      fontSize: 9,
      color: "#fff",
  ...
  }, srcOpen ? "▲" : "▼")), srcOpen && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 14
    }
  }, master.length === 0 ? /*#__PURE__*/React.createElement("label", {
    onDragOver: e => {
      e.preventDefault();
      setOver(true);
    },
    ...
    style: {
      display: "block",
      border: `2px dashed ${over ? ACCENT : "#e0e0e0"}`,
      borderRadius: 12,
      padding: "24px 16px",
      textAlign: "center",
      cursor: "pointer",
      background: over ? "#eef4ff" : "#fafafa"
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
  ...
  }, srcShown.length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 36,
      textAlign: "center",
      color: "var(--faint)"
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, it.bcode), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 9.5,
        color: cc,
        fontWeight: 800,
      ...
      style: {
        flexShrink: 0,
        border: "none",
        background: tagEditFor === it.bcode ? "var(--soft)" : "transparent",
        color: "var(--sub)",
        padding: "0 12px",
        cursor: "pointer",
        fontSize: 16
  ...
  }, [2, 3, 4].map(c => /*#__PURE__*/React.createElement("button", {
    key: c,
    onClick: () => setCols(c),
    style: {
      ...seg(cols === c),
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, it.name || it.bcode), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 6,
        flexWrap: "wrap"
      ...
      return /*#__PURE__*/React.createElement("button", {
        key: c.key,
        onClick: () => setCompanyFor(it.bcode, on ? null : c.key),
        title: c.name,
        style: {
          display: "flex",
          alignItems: "center",
