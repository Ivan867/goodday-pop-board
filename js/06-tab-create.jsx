...
function PopCreateInner({ seed, onSeedConsumed }) {
  ...
  const [qs, setQs] = useState({ fish:"", origin:"", catchcopy:"", appeal:"", price:"", recipes:"", size:"縦A4" });
  const [qsPrompt, setQsPrompt] = useState("");
  const [qsCopied, setQsCopied] = useState(false);
  const qsSet = (k, v) => setQs(o => ({ ...o, [k]: v }));
...
■仕上がり：超高解像度、フォトリアル、広告写真品質。チラシ風・漫画風・不自然な魚の形は避ける。重要な文字や価格が端で切れないよう余白を確保。文字は正確な日本語で描く。`;
  ...
  const qsCopy = async () => {
    try { await navigator.clipboard.writeText(qsPrompt); setQsCopied(true); setTimeout(()=>setQsCopied(false), 1800); } catch(e) {}
  };
        ...
        <div style={{ marginTop:14, background:"#141414", border:"1px solid #2e2a1a", borderRadius:12, padding:"15px 15px 17px" }}>
          ...
          <div style={{ display:"flex", gap:7, marginBottom:13 }}>
            {[["wamodern","和モダン","墨絵・和紙・上品"],["navygold","紺×金 高級","寿司・ハレの日"],["kirimi","切身リアル","写真風・産地推し"]].map(([k,l,d]) => {
              const on = qsStyle === k;
              return (
                <button key={k} onClick={()=>setQsStyle(k)}
                  style={{ flex:1, border: on?"2px solid #c8a840":"1px solid #2a2a2a", background: on?"#231f0f":"#1a1a1a", color: on?"#e6c860":"var(--text)", borderRadius:10, padding:"9px 4px", cursor:"pointer" }}>
                  <div style={{ fontSize:12.5, fontWeight:900 }}>{l}</div>
                  <div style={{ fontSize:9.5, opacity:0.7, marginTop:2 }}>{d}</div>
                </button>
              );
            })}
          </div>
          ...
          <div style={{ display:"flex", gap:7, marginBottom:12 }}>
            {["縦A4","横A4"].map(v => (
              <button key={v} onClick={()=>qsSet("size", v)}
                style={{ flex:1, border: qs.size===v?"2px solid #c8a840":"1px solid #2a2a2a", background: qs.size===v?"#231f0f":"#1a1a1a", color: qs.size===v?"#e6c860":"var(--text)", borderRadius:8, padding:"8px", fontSize:12.5, fontWeight:800, cursor:"pointer" }}>{v}</button>
            ))}
          </div>

          {qsPrompt && (
            <textarea value={qsPrompt} readOnly rows={6}
              style={{ width:"100%", boxSizing:"border-box", padding:"11px 12px", background:"#0f0f0f", border:"1px solid #2a2a2a", borderRadius:8, color:"var(--text)", fontSize:12, lineHeight:1.6, resize:"vertical", marginBottom:10 }} />
          )}
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={qsGen}
              style={{ flex:1.4, padding:"13px", borderRadius:8, border:"none", background: qs.fish.trim()?"#c8a840":"#333", color: qs.fish.trim()?"#141414":"#888", fontSize:14, fontWeight:900, cursor:"pointer" }}>
              プロンプト生成
            </button>
            <button onClick={qsCopy} disabled={!qsPrompt}
              style={{ flex:1, padding:"13px", borderRadius:8, border:"none", background: qsCopied?"#16a34a":(qsPrompt?"#8B6914":"#333"), color:"white", fontSize:14, fontWeight:700, cursor: qsPrompt?"pointer":"default", opacity:!qsPrompt?0.4:1 }}>
              {qsCopied?"コピー済":"コピー"}
            </button>
          </div>
