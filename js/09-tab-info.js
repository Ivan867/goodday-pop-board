...
function TodayInfoCard() {
  ...
  }, "今日の情報"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11.5,
      color: "var(--ink)",
      fontWeight: 800
    }
  }, jd(now), "（", wd(now), "）", /*#__PURE__*/React.createElement("span", {
    style: {
      color: "var(--sub)",
      fontWeight: 800
    }
  }, "\u3000出雲市"), hol && hol.todayName ? `・今日は「${hol.todayName}」🎌` : "", events.todayEv ? `・今日は「${events.todayEv.name}」${events.todayEv.food ? "〈" + events.todayEv.food + "〉" : ""}` : "")), (() => {
    const chips = [];
