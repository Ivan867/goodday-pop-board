...
function UploadModal({ currentStore, onClose, onSuccess }) {
  ...
  const pdfToImage = async (file) => {
    ...
    catch (e) { blob.name = name; return blob; }   // File が作れない環境では Blob をそのまま使う
  };
  ...
  const onFile = e => { const fl = Array.from(e.target.files || []); e.target.value = ""; addFiles(fl); };
          ...
          <div>
            ...
            <label {...dzImg.props} style={{ display:"block", position:"relative", overflow:"hidden", border:"2px dashed #e0e0e0", borderRadius:12, padding:"14px", textAlign:"center", cursor:"pointer", background:"#fafafa", ...dzImg.style }}>
              <div style={{ color:"var(--sub)", fontSize:14, fontWeight:700 }}>
                {dzImg.over ? "ここに離してください" : items.length ? "＋ さらに追加する" : "タップして選択（PDFもOK・まとめて選べます）"}
              </div>
              <input type="file" accept="image/*,.pdf,application/pdf" multiple onChange={onFile}
...
function PopDetail({ pop, onClose, onDelete, onLiked, onCommented, onCreateFromPop, navList, onNav }) {
  ...
  const doPrint = (perPage) => {
    ...
    const layout = perPage === 1
      ? `.sheet{display:block}.cell{width:210mm;height:297mm}`
      : perPage === 2
      ? `.sheet{display:flex;flex-direction:column}.cell{width:210mm;height:148.5mm}`
      : `.sheet{display:flex;flex-wrap:wrap}.cell{width:105mm;height:148.5mm}`;
    const cells = Array.from({ length: perPage }, () => `<div class="cell"><img src="${url}"></div>`).join("");
...
<\/script></body></html>`;
  ...
  const BarBtn = ({ onClick, icon, label, active, activeColor, fillWhenActive, primary, small, danger }) => (
    <button onClick={onClick} aria-label={label} aria-pressed={active ? true : undefined}
      style={{ flex:1, minWidth:0, minHeight: small ? 40 : 52, display:"flex", flexDirection:"column",
      ...
      <span style={{ fontSize: small ? 11 : 11, fontWeight:800, whiteSpace:"nowrap",
  ...
  return (
    <div data-popdetail="1" style={{ position:"fixed", inset:0, background:"rgba(15,25,38,0.62)", backdropFilter:"blur(3px)", WebkitBackdropFilter:"blur(3px)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000 }} onClick={onClose}>
      ...
      {showPrint && (
        <div onClick={() => setShowPrint(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {[
              [1, "A4に1枚", "1ページいっぱいに大きく"],
              [2, "A4に2枚", "上下に半分ずつ（A5サイズ）"],
              [4, "A4に4枚", "4分割（A6サイズ）"],
            ].map(([n, title, desc]) => (
            ...
            <button onClick={() => setShowPrint(false)}
              style={{ width:"100%", border:"none", background:"var(--chip)", color:"var(--text)", borderRadius:11, padding:"12px", fontSize:14, fontWeight:800, cursor:"pointer", marginTop:6 }}>やめる</button>
          </div>
        </div>
      )}
      ...
      {showDelConfirm && (
        ...
        {/* 操作バー（画像の下） */}
        <div style={{ borderBottom:"1px solid var(--line)", padding:"7px 8px 8px", flex:"0 0 auto" }}>
          <div style={{ display:"flex", gap:6 }}>
            ...
            <BarBtn onClick={() => { setShowDelConfirm(true); setPwInput(""); setPwError(""); }} icon="trash" label="消す" danger />
          </div>
        </div>

        {/* コメント欄（画像の下・シート内） */}
        <div id="pd-comments" style={{ padding:"8px 14px calc(10px + env(safe-area-inset-bottom))", flex:"0 1 auto", overflowY:"auto", WebkitOverflowScrolling:"touch", minHeight:44 }}>
...
function popShape(p) {
  ...
  const d = (p.img_w && p.img_h) ? { w: p.img_w, h: p.img_h } : ((window.__popDims || {})[id] || null);
...
function pairByShape(list, cols, K) {
  ...
  while (rest.length) {
    const a = rest.shift(), sa = popShape(a), row = [a];
    // 同じ形の相手を近くから探す
    for (let i = 0; i < Math.min(K, rest.length) && row.length < cols; ) {
      if (popShape(rest[i]) === sa) { row.push(rest.splice(i, 1)[0]); } else i++;
    }
    // 足りなければ、続きのものでうめる
    ...
    if (allLand) {
      ...
      const ars = row.map(x => {
        ...
        const d = (x.img_w && x.img_h) ? { w: x.img_w, h: x.img_h } : ((window.__popDims || {})[id] || { w: 1.41, h: 1 });
        return d.w / d.h;
      });
      const rowAr = Math.min.apply(null, ars);
      row.forEach(x => out.push({ ...x, __pairLand: true, __rowAr: rowAr }));
    } else {
...
;Object.assign(window, { PopCard, PopDetail, UploadModal, pairByShape, popShape });
