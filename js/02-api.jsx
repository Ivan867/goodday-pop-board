...
const api = {
  ...
  async topWow(weekStart) {
    const w = weekStart || (() => { const d = new Date(); const dow = d.getDay();
      d.setDate(d.getDate() - (dow === 0 ? 6 : dow - 1)); return d.toISOString().slice(0,10); })();
  ...
  async addTrendNote(term, species, context) {
    return sbJson(`/rest/v1/rpc/add_trend_note`, { method:"POST", body:{
      p_term: term, p_species: species || null, p_context: context || [],
  ...
  async groupPops(ids, name, cover) {
    if (!ids || !ids.length) return 0;
    return sbJson(`/rest/v1/rpc/admin_group_pops`, { method:"POST", body:{ p_ids: ids, p_name: name || null, p_cover: cover || null, p_password: PW_CACHE.admin || "" } });
  },
  async delMany(ids) {
    if (!ids || !ids.length) return 0;
  ...
  async verifyPasswordEx(purpose, password) {
    try {
      const v = await sbJson(`/rest/v1/rpc/check_secret_limited`, { method:"POST", body:{ p_kind: purpose, p_password: password } });
      if (v && v.ok) PW_CACHE[purpose] = password;
      return v || { ok:false, locked:false };
    } catch (e) {
      // 何かあっても従来のやり方で通す
  ...
  async getNotice() {
    const rows = await sbJson(`/rest/v1/site_notice?id=eq.1&select=enabled,message,tip_enabled,tip_message,feat_enabled,feat_message,feat_tab,feat_ver,badge_tab,badge_text,badge_ver,badge_until,menu_hidden,updated_at`);
    return rows[0] || { enabled:false, message:"", tip_enabled:false, tip_message:"季節のポップや時期が過ぎたポップは「アーカイブ」に収納されます。", feat_enabled:false, feat_message:"", feat_tab:"", feat_ver:"", badge_tab:"", badge_text:"", badge_ver:"", badge_until:null, menu_hidden:[] };
  },
  ...
  async listAllBundleItems() { return sbJson(`/rest/v1/pop_bundle_items?select=bundle_id,pop_id`); },
  ...
  async saveWeekLogs(logs) {
    ...
    return sbOne(`/rest/v1/order_logs`, { method:"POST", body:logs, prefer:"return=representation" });
  },
  ...
  async addOrderLog(o) { return sbOne(`/rest/v1/order_logs`, { method:"POST", body:o, prefer:"return=representation" }); },
  async deleteOrderLog(id) {
    const r = await sbFetch(`/rest/v1/order_logs?id=eq.${id}`, { method:"DELETE" });
    if (!r.ok) throw new Error(await r.text());
    return true;
  },

  // ── promo_plans：これからの販促予定 ──
  async listPlans() { return sbJson(`/rest/v1/promo_plans?select=*&order=start_date.asc.nullslast,created_at.desc`); },
  async addPlan(p) { return sbOne(`/rest/v1/promo_plans`, { method:"POST", body:p, prefer:"return=representation" }); },
  async updatePlan(id, patch) {
    return sbOne(`/rest/v1/promo_plans?id=eq.${id}`, { method:"PATCH", body:{ ...patch, updated_at:new Date().toISOString() }, prefer:"return=representation" });
  },
  async deletePlan(id) {
    const r = await sbFetch(`/rest/v1/promo_plans?id=eq.${id}`, { method:"DELETE" });
    if (!r.ok) throw new Error(await r.text());
    return true;
  },
