...
function App() {
  ...
  useEffect(() => {
    const measure = () => { if (headerRef.current) setHeaderH(headerRef.current.scrollHeight); };
    measure();
  ...
  return (
    <div id="app-scroll" style={{ background:"var(--bg)", paddingBottom:"calc(62px + env(safe-area-inset-bottom))" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:100, paddingTop:"env(safe-area-inset-top)", background:"var(--bg)", overflow:"hidden", height: headerH ? `${headerH*(1-scrollP)}px` : "auto" }}>
        <div ref={headerRef} style={{ position:"relative", maxWidth:1080, margin:"0 auto", padding:"6px 16px 3px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"nowrap", gap:10, opacity: 1-scrollP, transform:`translateX(${-scrollP*40}px)`, pointerEvents: scrollP>0.7?"none":"auto" }}>
