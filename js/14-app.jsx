  ...
  return (
    <div id="app-scroll" style={{ background:"var(--bg)", paddingBottom:"calc(70px + env(safe-area-inset-bottom))" }}>
      ...
      <div style={{ position:"fixed", left:0, right:0, bottom:0, zIndex:205, display:"flex", justifyContent:"center", padding:0, pointerEvents:"none" }}>
       <div style={{ display:"flex", alignItems:"center", justifyContent:"space-around", gap:2, width:"100%", maxWidth:1600, background:"var(--primary)", border:"none", borderRadius:"22px 22px 0 0", boxShadow:"0 -2px 14px rgba(20,40,70,0.16)", padding:"6px 8px calc(7px + env(safe-area-inset-bottom, 0px) / 2)", pointerEvents:"auto" }}>
        {[tabs[0], { key:"bundle", icon:"📅", label:"行事", color:"#3f7cb0" }, { key:"catalog", icon:"📖", label:"カタログ", color:"#b8860b" }, { key:"__more", icon:"≡", label:"メニュー", color:"#6b7280", more:true }].map(({key,icon,label,color,action,more,filter})=>{
