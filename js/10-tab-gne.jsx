...
function GeneratorTab({ onCreatePop }) {
  ...
  const [land, setLand] = useState(() => { try { return localStorage.getItem("gneLand") === "1"; } catch(e) { return false; } });
  const setLandSave = (v) => { setLand(v); try { localStorage.setItem("gneLand", v ? "1" : "0"); } catch(e) {} };
  ...
  const renderBlob = (row) => new Promise((res) => {
    ...
    gneRender(c.getContext("2d"), row, tpl, taxMode, font, taxRate, { x: gx, y: gy, scale: gScale / 100, fieldScale: fScale, fieldPos: fPos }, { w: CW, h: CH });
      ...
      <div className="gne-grid" style={{ display:"grid", gridTemplateColumns:"minmax(0, 1fr)", gap:14 }}>
        ...
        <div style={card}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
              ...
              <div style={{ marginLeft:"auto", display:"flex", gap:2, background:"rgba(120,120,128,0.12)", borderRadius:8, padding:2 }}>
                {[[false, "たて"], [true, "よこ"]].map(([v, l]) => (
                  <button key={l} onClick={() => setLandSave(v)} aria-pressed={land === v}
                    style={{ border:"none", background: land === v ? "#fff" : "transparent", color: land === v ? "var(--ink)" : "var(--sub)",
                      borderRadius:6, padding:"4px 12px", fontSize:11.5, fontWeight:800, cursor:"pointer",
                      boxShadow: land === v ? "0 1px 2px rgba(0,0,0,0.1)" : "none" }}>{l}</button>
                ))}
              </div>
            </div>
