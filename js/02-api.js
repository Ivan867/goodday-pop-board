...
const api = {
  ...
  async listAll() {
    return sbJson(`/rest/v1/pops?select=${POP_COLS}&deleted_at=is.null&order=created_at.desc`);
  },
  ...
  async addIdea(title, memo, images, tags) {
    const r = await sbFetch(`/rest/v1/rpc/admin_add_idea`, {
      method: "POST",
      body: {
        p_title: title,
        p_memo: memo || null,
  ...
  async deleteIdea(id) {
    const r = await sbFetch(`/rest/v1/rpc/admin_delete_idea`, {
      method: "POST",
      body: {
        p_id: id,
        p_password: PW_CACHE.admin || ""
  ...
  // ── バックアップ：中身をまるごと書き出す ──
  BACKUP_TABLES: ["pops", "pop_comments", "catalogs", "order_items", "order_sheets", "order_sheet_rows", "order_logs", "pop_bundles", "pop_bundle_items", "pop_bundle_prompts", "resources", "market_trends", "shared_tray_presets", "prompt_library", "fish_columns", "blog_posts", "pop_requests", "site_notice", "production_notes", "op_logs"],
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      try {
        ...
        for (;;) {
          let part;
          try {
            part = await sbJson(`/rest/v1/${t}?select=*&offset=${from}&limit=1000`);
          } catch (e) {
            part = [];
          }
  ...
  async listDeleted() {
    return sbJson(`/rest/v1/pops?select=${POP_COLS},deleted_at&deleted_at=not.is.null&order=deleted_at.desc`);
  },
  ...
  // アーカイブ済みのみ（アーカイブタブ用）。
  async listArchived() {
    return sbJson(`/rest/v1/pops?select=${POP_COLS}&archived=eq.true&deleted_at=is.null&order=created_at.desc&limit=${POP_LIMIT}`);
  },
  // 画像の縦横を記録（まだ測っていないものだけ）
  async setPopDims(id, w, h) {
    try {
      await sbFetch(`/rest/v1/rpc/set_pop_dims`, {
        method: "POST",
        body: {
          p_id: id,
          p_w: Math.round(w),
  ...
  async createSheet(o) {
    return sbOne(`/rest/v1/order_sheets`, {
      method: "POST",
      body: o,
      prefer: "return=representation"
    });
  ...
  async updateSheet(id, patch) {
    return sbOne(`/rest/v1/order_sheets?id=eq.${id}`, {
      method: "PATCH",
      body: {
        ...patch,
        updated_at: new Date().toISOString()
  ...
  async addSheetRow(o) {
    return sbOne(`/rest/v1/order_sheet_rows`, {
      method: "POST",
      body: o,
      prefer: "return=representation"
    });
  ...
  async updateSheetRow(id, patch) {
    return sbOne(`/rest/v1/order_sheet_rows?id=eq.${id}`, {
      method: "PATCH",
      body: patch,
      prefer: "return=representation"
    });
  ...
  async deleteSheetRow(id) {
    const r = await sbFetch(`/rest/v1/order_sheet_rows?id=eq.${id}`, {
      method: "DELETE"
    });
    if (!r.ok) throw new Error(await r.text());
    return true;
